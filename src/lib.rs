...
//! [JSON Feed][jsonfeed] Model provides types which can be used to manipulate JSON Feed data.
//!
//! The crate is basically a [newtype][newtype] wrapper around [Serde JSON][serde_json]'s `Map` type and provides
//! methods to JSON Feed properties.
//!
//! For example, a library user can have a slice of bytes and create a `Feed` by calling `from_slice`. If the
//! slice of bytes is a JSON object, then a `Feed` instance is returned. The only guarantee which `Feed` and
//! other model types make is that the JSON data is a JSON object.
//!
//! The library user can call `is_valid(Version::Version1_1)` on the `Feed` instance to determine if the JSON
//! object is a valid Version 1.1 JSON Feed.
//!
//! # Accessor Methods
//!
//! If the library user wants to read or write data, then methods like `title()`, `set_title(...)`, and
//! `remove_title()` exist on `Feed`.
//!
...
//! JSON string but the value is a JSON number, then an `Error::UnexpectedType` will be returned. The field
//! value may or may not be present so the `Option` type is used to indicate if a value exists.
//!
//! For "setter" and "remove" methods, any existing value in the JSON object is returned.
//!
//! # Owned, Borrowed, and Borrowed Mutable Types
//!
//! There are 3 variants of every model type, the "owned" data type (e.g. `Feed`), the borrowed data type (e.g.
//! `FeedRef`), and the borrowed mutable data type (e.g. `FeedMut`). In most cases, the "owned" data type will
...
//!
//! A few standard traits are implemented like `From<Map<String,Value>>` and `Serialize` as well as a few
//! helper methods like `as_map()` and `as_map_mut()` for the model types.
//!
//! # Examples
//!
